...
import { Box, Divider, Text, Flex, Checkbox, HStack, Image, Stack, Input, Grid, Button } from "@chakra-ui/react";
...
export default function Sidebar() {
    const { selectedMarker, searchOptions, setSearchOptions, cardRefs } = useMapContext();
    ...
    useEffect(() => {
        if (selectedMarker !== null) {
          const index = alertData.findIndex((item) => item.id === selectedMarker.id);
    ...
    return (
        <Box>
            ...
            <Flex flexWrap="wrap" gap={6}>
                {Object.entries(iconPerType).map(([label, imageUrl]) => (
                    <Flex key={label} alignItems="center">
                        <Checkbox defaultChecked onChange={(e) => handleCheckboxChange(label, e.target.checked)}>
            ...
            <Stack direction="column" mr='1'>
                <Text mt='2' textColor={'gray.200'}>Content Search</Text>
                <Input onChange={handleInputChange} bg="#232934" _hover={{ backgroundColor: '#272d38' }} _placeholder={{ color: 'gray.500' }} placeholder="Content Search..." variant="filled" textColor={"white"} />
            </Stack>
            ...
            <Grid templateColumns="1fr auto" alignItems="center">
                ...
                <Button
                    ...
                    ml="1"  mt="1" colorScheme={"cyan"} size="sm" lineHeight="1">
                    Search
                </Button>
            </Grid>

            <Text mt='2' textColor={'gray.200'}>Result</Text>
            <Box maxHeight="calc(100vh - 395px)" overflow="auto">
                {alertData.map((item: IAlert, id: number) => (
                    
                    <CardAlert
                        key={id}
                        ref={(el: HTMLElement) => (cardRefs.current[id] = el)}
                        alert={item}
                    />
                ))}
            </Box>
